...
def findbins(nparray, method=None):
    # numarray : array_like
    ...
    #default to FD if no method is specified, or if FD is explicitly selected
    if not method or (method == "FD"):
        # WIDTH-OF-BINS method
        #  Freedman-Diaconis
        ...
        nN = (num ** (-1. / 3.))
        # get array containing quantiles
        quant = mquantiles(nparray, prob=[0.25, 0.75])
        # IQR is the interquartile range
        # (the 75th percentile minus the 25th percentile).
        IQR = quant[1] - quant[0]
        # produce Median Absolute Deviation
        nmad = mad(nparray)
        #  If IQR is zero and mad(x) is nonzero,
        # then the mad value is used instead of the IQR.
        if ((IQR == 0) and (nmad != 0)):
            estimate = (2 * nmad * nN)
        #  If IQR is zero and mad(x) is zero,
        # and when that is 0 as well, returns size of the whole range
        # (1 effective bin)
        elif ((IQR == 0) and (nmad == 0)):
            estimate = (maxval - minval)
        else:
            estimate = (2 * IQR * nN)
    elif (method == "ST"):
        # NUMBER-OF-BINS method
        # Sturges
        #  Sturges, H. A. (1926). "The choice of a class interval".
        # Journal of the American Statistical Association: 65–66.
        # doi:10.1080/01621459.1926.10502161. JSTOR 2965501.
        # See also work of "Rob Hyndman"
        # http://www.robjhyndman.com/papers/sturges.pdf
        estimate = np.float((np.log2(num) + 1))
    elif (method == "SR"):
        # NUMBER-OF-BINS method
        # Square root
        # we decide the bins with square root method
        # estimate = int(np.ceil(np.float(np.sqrt(len(numarray))))) <-- SLOWER
        estimate = np.float((len(nparray) ** 0.5))
    elif (method == "RC"):
        # NUMBER-OF-BINS method
        # Rice
        ...
        estimate = np.float((2 * (num ** (1 / 3))))
    elif (method == "SD"):
        # NUMBER-OF-BINS method
        # Doane modification of Sturges formula
        # D. Wilkinson, "The Grammar of Graphics", 2005.
        # chapter 7, section "Bins", sub-section "Ordinary Histogram": 133-134
        estimate = np.float((3 + (np.log10(num) * np.log2(num))))
    elif (method == "DA"):
        # NUMBER-OF-BINS method
        # Doane
        # Doane DP (1976) Aesthetic frequency classiﬁcation.
        # American Statistician, 30: 181–183
        from scipy.stats import skew
        g1 = np.abs(skew(nparray))
        sigma = np.sqrt((6. * (num - 2.)) / ((num + 1.) * (num + 3.)))
        estimate = np.float((1 + np.log2(num) + np.log2(1 + (g1 / sigma))))
    elif (method == "SC"):
        # WIDTH-OF-BINS method
        # Scott
        # http://biocurious.com/files/histo-bin-size-scott.pdf
        # WARNING - discrepancies in literature
        # original formula        --> "W = 3.49 * S * N(-1/3)"
        # used by R, scipy, etc.. --> "h = 3.5 * S * N(-1/3)"
        # USING "h = 3.5 * S * N(-1/3)"
        # W is the width of the histogram bin
        # S (sigma) is the standard deviation of the distribution
        # N is the number of available samples
        const = 3.5
        sigma = np.std(nparray)
        nN = (num ** (-1. / 3.))
        estimate = (const * sigma * nN)
        #  If the bandwidth is non-positive
        # the number of bins is set to size of the whole range (1 effective bin)
        if (estimate <= 0):
            estimate = (maxval - minval)
    else:
        raise ValueError("ERROR: Unknown binning method.")

    return(np.float(estimate), minval, maxval)

def autobin(nparray, method=None, binning=False):
    # NUMBER-OF-BINS methods
    # - Sturges              ---> ST (also in R)
    # - Square Root          ---> SR
    # - Rice                 ---> RC
    # - Sturges-Doane        ---> SD
    # - Doane                ---> DA

    # WIDTH-OF-BINS methods
    # - Scott                ---> SC (also in R)
    # - Freedman–Diaconis    ---> FD (also in R)

    nparray = np.sort(nparray)
    # get estimated bin size
    estimate, minval, maxval = findbins(nparray, str(method))
    # MISSING METHODS
    # - Shimazaki H. and Shinomoto S., Kernel Bandwidth Optimization
    # http://toyoizumilab.brain.riken.jp/hideaki/res/kernel.html
    # http://toyoizumilab.brain.riken.jp/hideaki/res/code/sskernel.py

    # - K. Knuth, "Optimal Data-Based Binning for Histograms", 2006.
    # http://arxiv.org/pdf/physics/0605197v1.pdf
    # https://github.com/rootpy/rootpy/blob/master/rootpy/plotting/autobinning.py
    ...
    # - Sturges-Doane
    # - Doane
    number_of_bins = ["ST", "SR", "RC", "SD", "DA"]
    # WIDTH-OF-BINS methods
    # - Scott
    # - Freedman–Diaconis
    width_of_bins = ["SC","FD"]
    if (method in number_of_bins):
        nbins = int(np.ceil(estimate))
        # calculate bin size as np float
        binsize = ((maxval - minval) / nbins)
    elif (method in width_of_bins):
        nbins = int(np.ceil((maxval - minval) / estimate))
        # calculate bin size as np float
        binsize = estimate
    else:
        raise ValueError("ERROR: Unknown binning method.")
    ...
    if binning:
        ...
        return binsize, minval, maxval, nbins, binned
    else:
        return binsize, minval, maxval, nbins
...
def lag_plot(y1, y2, lag, filename=None,
                          title=None,
                          xlabel=None,
                          ylabel=None):
    ...
    if not title:
        ax.set_title("Lag Plot",fontsize=14)
    else:
...
def prepare_lagplot(series, lag=None):
    if not lag:
        lag = 1
    ...
    for num in series:
        ydata.append(num)
    # For example, for lag "-1", observations x2, x3, xn
    # are plotted against observations x1, x2, xn-1
    last = len(ydata) - lag
    # create new list without last (n = lag) elements
    # this is a COPY of the original list
    xy1 = ydata[:last]
    # we change the original list IN PLACE, this is NOT a copy
    del(ydata[:lag])
    xy2 = ydata
    # remove duplicates by creating dictionary
    dedup = dict(zip(xy1, xy2))
    # split dictionary in two lists so can be used for scatter plot
    y1 = list()
    y2 = list()
    for k,v in dedup.items():
        y1.append(k)
        y2.append(v)
    # return each individual list
    return y1, y2

def run_plot(y1, y2, filename=None,
                     title=None,
                     xlabel=None,
                     ylabel=None):
    ...
    if not title:
        ax.set_title("Run-Sequence Plot",fontsize=14)
    else:
        ax.set_title(title, fontsize=14)
...
def histogram(series, filename=None,
                      title=None,
                      xlabel=None,
                      ylabel=None):

    binsize, minval, maxval, nbins = autobin(series, method='ST', binning=False)
    ...
    plt.hist(series, bins=nbins, normed=False, histtype='bar')
...
def acorr_plot_twoside(data, filename=None,
                             title="ACR Plot",
                             xlabel="Lag #",
                             ylabel="Correlation"):
    ...
    plt.acorr(data, detrend=mlab.detrend_mean,
                    usevlines=True,
                    maxlags=None,
                    normed=True,
                    lw=2)
...
def acorr_plot_oneside(x, ax=None):
    # http://stackoverflow.com/questions/27541290/bug-of-autocorrelation-plot-in-matplotlib-s-plt-acorr
    fig, ax = plt.subplots(figsize=(12, 8))
...
data = pd.concat([data, pd.DataFrame(columns=['LATITUDE_SOUTH_180'])], ignore_index=True)
...
for index, row in data.iterrows():
    ...
    # create numerical "LATITUDE_CIRCLE_IMAGE"
    clat = row["LATITUDE_CIRCLE_IMAGE"]
    elat = clat + 90
    # set new value
    data.set_value(index, 'LATITUDE_SOUTH_180', elat)
...
data["LATITUDE_SOUTH_180"] = data["LATITUDE_SOUTH_180"].astype('float64')
###############################################################################
# WEEK 2
...
nsum = 0
count = 0
for num in data["DIAM_CIRCLE_IMAGE"]:
...
# ANOVA- "DIAM_CIRCLE_IMAGE" ~ "DIAM_CENTERED"
anova = data[["DEPTH_RIMFLOOR_TOPOG", "DIAM_CENTERED"]]

# ANOVA
model1 = smf.ols(formula='DEPTH_RIMFLOOR_TOPOG ~ DIAM_CENTERED', data=anova)
results1 = model1.fit()
print("Performing ANOVA analysys between 'DEPTH_RIMFLOOR_TOPOG' and 'DIAM_CENTERED'")
print()
...
ax.set_xlabel("Crater Diameter (mean centered)",fontsize=12)
ax.set_ylabel("Crater Depth (km)",fontsize=12)
